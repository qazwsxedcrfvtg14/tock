...
pub struct StoragePermissions {
    ...
    /// (update) access to.
    write_permissions: [NonZeroU32; 8],
...
impl StoragePermissions {
    ...
    pub fn new_kernel_permissions(_cap: &dyn capabilities::SuperuserStorageCapability) -> Self {
        let read_permissions: [NonZeroU32; 8] = [NonZeroU32::new(1).unwrap(); 8];
        let write_permissions: [NonZeroU32; 8] = [NonZeroU32::new(1).unwrap(); 8];
        StoragePermissions {
            read_count: 0,
            read_permissions,
            write_count: 0,
    ...
    pub fn check_read_permission(&self, storage_id: u32) -> bool {
        if self.superuser {
            ...
            // Otherwise check if given storage_id is in read permissions array.
            let sid = NonZeroU32::new(storage_id);
            sid.map_or(false, |sid| {
